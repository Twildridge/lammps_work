...
- *2026-04-16*: Fixed `shear_slab.lmp` triclinic box error. `fix deform xz erate` requires a triclinic box; the isolated gel data file creates an orthogonal box. Fix: added `change_box all triclinic` immediately before `fix deform_xz` in Phase 2. This converts the box to triclinic (xz=0 initially) without changing its shape, allowing tilt factors to be applied.
